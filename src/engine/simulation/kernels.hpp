...
inline glm::vec3 grad_poly6(const glm::vec3 &r, float h) {
    ...
    return glm::vec3(static_cast<float>(-6.0f * factor * powf(h * h - r_len * r_len, 2.0f)) * r);
}
...
inline glm::vec3 grad_viscosity(const glm::vec3 &r, float h) {
    ...
    const float dw_dr =
        factor * (-3.0f * r_len * r_len / (2.0f * powf(h, 3)) + 2.0f * r_len / (h * h) - h / (2.0f * r_len * r_len));
...
inline float poly6_kernel(const glm::vec3 &r, float h) {
    ...
    if (0 <= r_len && r_len <= h) {
        const float factor = 315 / (64 * std::numbers::pi_v<float> * powf(h, 9));
...
inline float viscosity_kernel(const glm::vec3 &r, float h) {
    ...
    if (r_len < EPSILON || r_len > h) return 0.0f;
    if (0 <= r_len && r_len <= h) {
